<section class="hero-wrap js-fullheight" style="height: 653px;">
  <div class="overlay"></div>
  <div class="container">
    <div class="row no-gutters slider-text js-fullheight justify-content-center align-items-center" style="height: 653px;">
      <div class="col-lg-8 col-md-6 ftco-animate d-flex align-items-center fadeInUp ftco-animated">
        <div class="text text-center">
          <span class="subheading">Hey! I am</span>
          <h1>Anglebert Shumbusho</h1>
          <h2>I'm a
            <span class="txt-rotate" data-period="2000" data-rotate="[ &quot;Web Developer.&quot;, &quot;Designer.&quot;, &quot;Photographer.&quot;, &quot;Marketer.&quot;]"><span class="wrap">Developer.</span></span>
          </h2>
        </div>
      </div>
    </div>
  </div>

  <div class="mouse">
    <a href="#" class="mouse-icon">
      <div class="mouse-wheel"><span class="ion-ios-arrow-round-down"></span></div>
    </a>
  </div>
</section>